...
class Trainer:
    ...
    def get_optimizer(self):
        ...
        params = [{"params": rhos, "lr": self.lr, "weight_decay":0.0},{'params': params}]
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            if(len(log) > 1):
                if(log[1] <= self.min_validation_loss):
                    self.bad_validation = 0
                    self.min_validation_loss = log[1]
                    self.save_model(best = True)
                else:
                    self.bad_validation += 1
                    if(self.bad_validation >= self.patience):
                        break
